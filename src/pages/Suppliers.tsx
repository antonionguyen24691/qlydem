  ...
  return <div className="flex h-full flex-col bg-zinc-50">
    <div className="flex flex-col gap-3 border-b border-zinc-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      ...
      {canManageInventory(user) && <div className="grid grid-cols-[minmax(0,1fr)_44px_44px] gap-2 sm:flex sm:items-center">
        ...
        {isAdmin(user) && <>
          ...
          <label className={`inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-[var(--radius-control)] border border-zinc-200 bg-white text-zinc-900 shadow-sm hover:bg-zinc-50 sm:h-10 sm:w-auto sm:px-3 ${isImporting ? "pointer-events-none opacity-50" : ""}`} title="Nhập XLSX">
    ...
    <Dialog isOpen={!!payingSupplier} onClose={() => setPayingSupplier(null)} title="Thanh toán công nợ nhà cung cấp">
      {payingSupplier && <form onSubmit={paySupplier} className="space-y-4">
        <div className="rounded-[var(--radius-control)] bg-zinc-50 p-3"><div className="font-bold text-zinc-900">{payingSupplier.name}</div><div className="mt-1 text-sm text-red-600">Còn phải trả: {Number(payingSupplier.current_payable ?? 0).toLocaleString()} ₫</div></div>
        ...
        <div><label className="mb-1.5 block text-sm font-bold text-zinc-700">Nguồn tiền</label><select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as "CASH" | "TRANSFER")} className="h-11 w-full rounded-[var(--radius-control)] border border-zinc-200 bg-white px-3"><option value="CASH">Tiền mặt</option><option value="TRANSFER">Chuyển khoản</option></select></div>
