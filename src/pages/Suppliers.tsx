  ...
  return <div className="flex h-full flex-col bg-zinc-50">
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6">
      <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
        ...
        <tbody className="divide-y divide-zinc-100">{visible.map((supplier) => <tr key={supplier.id} className="hover:bg-zinc-50"><td className="px-4 py-4"><div className="font-bold text-zinc-900">{supplier.name}</div><div className="mt-1 text-xs text-zinc-500">{supplier.code}</div></td><td className="px-4 py-4 text-zinc-600">{supplier.contact_person || supplier.phone || "-"}<div className="text-xs text-zinc-400">{supplier.phone || ""}</div></td><td className="px-4 py-4 text-zinc-600">{supplier.tax_code || "-"}</td><td className="px-4 py-4 text-right font-bold text-red-600">{Number(supplier.current_payable ?? 0).toLocaleString()} ₫</td><td className="px-4 py-4 text-center"><span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">{supplier.status === "ACTIVE" ? "Đang dùng" : "Ngừng dùng"}</span></td><td className="px-4 py-4 text-right">{canManageInventory(user) && <Button size="sm" variant="outline" onClick={() => openEdit(supplier)}><Edit3 className="mr-1 h-3.5 w-3.5" />Sửa</Button>}</td></tr>)}</tbody></table></div>
